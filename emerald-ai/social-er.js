...
function buildSocialERHtml(erResults) {
  ...
  const statCards = [
    ...
    <div style="flex:1;min-width:150px;background:#181e2e;border:1px solid #252d40;border-radius:8px;padding:16px 18px">
  ...
  const orgRows = erResults.map(r => {
    ...
    const postSnippets = (platform, items, linkCol) => items.slice(0, 2).map(s => `
      <div style="margin-top:8px;padding:8px 10px;background:#0a0e17;border-left:2px solid #252d40;border-radius:0 4px 4px 0">
        ...
        ${s.link ? `<a href="${escHtml(s.link)}" style="font-size:10px;color:${linkCol};text-decoration:none;display:inline-block;margin-top:4px" target="_blank">↗ ${platform}</a>` : ''}
      </div>`).join('');
...
<section class="sec" id="social">
  <div class="sh">
    ...
    <div class="sd">Google-indexed posts mentioning each organisation in an air quality context on LinkedIn, X/Twitter, and Instagram. Scored 0–10 on a transparent formula: Volume (0–4) + Platform Breadth (0–3) + AQ Keyword Relevance (0–3).</div>
    <div class="sdiv"></div>
  </div>
  ...
  <div style="background:rgba(74,159,212,.06);border:1px solid rgba(74,159,212,.18);border-radius:8px;padding:12px 16px;margin-bottom:24px;font-size:12px;color:#8fa3b8;line-height:1.7">
    ...
    <strong>Breadth</strong> (0–3): 1pt per platform (LinkedIn / X / Instagram) with ≥1 indexed post &nbsp;·&nbsp;
